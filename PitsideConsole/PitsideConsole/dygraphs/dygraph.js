...
Dygraph.prototype.mouseMove_ = function(event) {
  ...
  var highlightOnHover = this.attr_("highlightOnHover");
  ...
  if (this.selPoints_.length > 0) {
    ...
    if (this.attr_("showVerticalCrosshair")) {
      ...
      if (this.attr_("crosshairColor")) {
        color = this.attr_("crosshairColor");
      }
      if (this.attr_("crosshairWidth")) {
        width = this.attr_("crosshairWidth");
      }
      ctx.strokeStyle = color;
      ctx.lineWidth = width;
      ctx.beginPath();
      ctx.moveTo(canvasx, 0);
      ctx.lineTo(canvasx, this.height_);
      ctx.closePath();
      ctx.stroke();
    }
    ctx.restore();
  ...
  if (row !== false && row >= 0) {
    ...
    for (var setIdx = 0; setIdx < this.layout_.datasets.length; ++setIdx) {
      ...
      if (row < set.length) {
        ...
        } else if (this.attr_("interpolatePoints")) {
          // Interpolate missing data here...
          // step 1: find good points before and after
          var beforePnt = null;
          var afterPnt = null;
          for (var rowi = row; rowi >= 0; rowi--) {
            var testPnt = this.layout_.points[pos+rowi];
            if (testPnt.yval !== null && testPnt.name === point.name) {
              beforePnt = testPnt;
              break; 
            }
          }
          for (var rowi = row; rowi < set.length; rowi++) {
            var testPnt = this.layout_.points[pos+rowi];
            if (testPnt.yval !== null && testPnt.name === point.name) {
              afterPnt = testPnt;
              break; 
...
//           this.selPoints_.push(beforePnt);
        ...
        // data before the first or after the last lap position is useless!
          ...
          var canvasy_delta = afterPnt.canvasy - beforePnt.canvasy;
          var xRatio = (point.canvasx - beforePnt.canvasx) / canvasx_delta;
          var ySegment = xRatio * yval_delta;
          point.yval = beforePnt.yval + ySegment;
          point.canvasy = beforePnt.canvasy + (xRatio * canvasy_delta);
          this.selPoints_.push(point); 
          // step 3: build and push the object to selPoints
